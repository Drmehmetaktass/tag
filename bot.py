  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("[{usr.first_name}](tg://user?id={usr.id}) İşlem Başarılı Bir Şekilde Durduruldu ❌")
        return
      if usrnum == 5:
